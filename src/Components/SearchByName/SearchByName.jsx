...
import noImageResult from "../../assests/noSearchResults.jpg"
...
export default function SearchByName() {
  ...
  let [loaderVal,setLoaderVal]=useState(false)
    ...
    let [fromSearchItems,setFromSearchItems]=useState([])

    let SearchedVal
    
    async function searchByName(SearchedVal)
    {
      setLoaderVal(true)


        let {data} =await axios.get(`https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${SearchedVal}`)

        console.log(data.drinks)

        setFromSearchItems(data.drinks)
    ...
    function getSearchedVal()
    {
         SearchedVal =document.querySelector(".searchInput").value
        searchByName(SearchedVal)
    ...
    async function getDrinkId(DrinkId,relatedDrinksDetailed)
    {
        ...
        getSelctedCategory(relatedDrinksDetailed)
...
//   for related drinks
let [relatedDrinkImages,setRelatedDrinkImages]=useState([])

async function getSelctedCategory(productNameParameter)
{
    if(productNameParameter=="Other")
     {

        productNameParameter="Other / Unknown"
     } 
     else if(productNameParameter=="Punch")
    {
        productNameParameter="Punch / Party Drink"

    }
    else if(productNameParameter=="Coffee")
     {
        productNameParameter="Coffee / Tea"

     }
     console.log(productNameParameter)

    let {data}=await axios.get(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=${productNameParameter}`)

    console.log(data.drinks)

    setRelatedDrinkImages(data.drinks)
}
...
<title>Search  page  </title>
  ...
  <h2 className='text-center mt-2 text-white'>Search For Drinks By Name</h2>
...
<div className="wrapper">
  <div className="search-input">
    ...
    <div className="icon" onClick={()=>{getSearchedVal()}}><i className="fas fa-search iconSearch"></i></div>
  </div>
</div>
...
<h3 className='my-5 text-center '>( Ingradiants )</h3>
  ...
  {el.strIngredient1!=""&&el.strIngredient1!=null?
            ...
            {fromSearchItems.length!=1&&fromSearchItems?
            <>
                      <div  className='bg-white w-75 p-1 mt-5 rounded-pill'></div>

                      <h3 className='my-5 text-center '>Related Drinks  </h3>
...
{relatedDrinkImages.length!=1&&relatedDrinkImages?relatedDrinkImages.map((el)=>{
...
{/* <h3 className='text-warning text-center'>Nothing</h3> */}
    ...
    <div className="row g-4 justify-content-center">
        ...
        {/* <h3 className='text-center'>  <span className='text-danger'>( {aboutDrinkSearch} ) <span className='text-white'>Drinks</span></span> </h3> */}

    {fromSearchItems!=[]&&fromSearchItems!=null?fromSearchItems?.map(el=>{

        return <>
        {/* ////////////////////// */}
        {/* "col-sm-6 col-lg-4" for mor than 2 */}
        <div    className={fromSearchItems.length==1?"col-lg-6":fromSearchItems.length==2?"col-lg-6":"col-md-6 col-xl-4"}>
        <div className="categoryItem p-3">
            <img src={el.strDrinkThumb} alt={el.strDrink}  onClick={()=>getDrinkId(el.idDrink,el.strCategory)} className="categoryItemImg" data-bs-toggle="modal" data-bs-target="#exampleModal"/>
    ...
    <img src={noImageResult} className='w-100 rounded-5' alt="" />
