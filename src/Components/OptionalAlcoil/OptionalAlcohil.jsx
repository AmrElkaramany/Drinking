...
export default function OptionalAlcohil() {
  let [loaderVal,setLoaderVal]=useState(false)

  let chosenFilter="Optional alcohol"
  let [glassItems,setGlassItems]=useState([])


  async function getSelctedGlass(productNameParameter)
  {
    setLoaderVal(true)


      let {data}=await axios.get(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?a=${productNameParameter}`)

      console.log(data.drinks)

      setGlassItems(data.drinks)
...
<Helmet>
<meta charSet="utf-8" />
<title> Optional alcohol Drinks  </title>
</Helmet>
<br />
<br />
<br />
...
{el.strIngredient1!=""&&el.strIngredient1!=null?
          ...
          {glassItems.length!=1&&glassItems?
          <>
                    <div  className='bg-white w-75 p-1 mt-5 rounded-pill'></div>

                    <h3 className='my-5 text-center '>Related Drinks in ( {chosenFilter} ) Category</h3>
...
{glassItems.length!=1&&glassItems?glassItems.map((el)=>{
...
{/* <h3 className='text-warning text-center'>Nothing</h3> */}
  ...
  <div className="row g-4 justify-content-center">
      <h3 className='text-center'> <span className='text-danger'>( {chosenFilter} )  <span className='text-white'>Drinks</span></span> </h3>

  {glassItems!=[]?glassItems?.map(el=>{
